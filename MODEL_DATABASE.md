...
Points-based scoring ladder (added 2026-07-07, commit `dc942e3`), mirrored in
`frontend/components/ui.tsx`: **5 pts** exact scoreline + correct winner,
**3 pts** correct winner only, **1 pt** actual result was a draw, **0 pts**
wrong winner. Replaced the old binary hit/miss tally everywhere accuracy is
shown (news ticker, matches-page accuracy panels, knockout-tie verdicts).
Current numbers in Section 12.
...
Accuracy is now scored with the **points-based ladder** (Section 1.10, commit
`dc942e3`, 2026-07-07), which replaced the old binary hit/miss tally used
below through v1.1 of this doc. Per match: **5 pts** exact scoreline + correct
winner, **3 pts** correct winner only, **1 pt** actual result was a draw,
**0 pts** wrong winner. Max possible = 5 × matches played.

### 12.1 Current Totals (as of Jul 7, 95 matches played)

| Scope | Points | Max | Accuracy |
|-------|--------|-----|----------|
| **Overall** (72 group + 23 knockout) | 241 | 475 | **51%** |
| Group stage only | 170 | 360 | 47% |
| **Knockout stage** (R32 + R16 so far) | 71 | 115 | **62%** |

Source: `frontend/public/snapshot/api_news.json` ticker
(`"CAI accuracy: 241/475 pts (51%) · KO: 71/115 pts (62%)"`), regenerated by
`gen_snapshots.py` after every result ingest. Knockout accuracy running ahead
of group-stage accuracy tracks with the form-led thesis (Section 1) — teams
that are still in the tournament are, by construction, in better recent form
than the ones already eliminated.
...
|-------|---------|--------|-----|----------|
| Round of 32 | 16 | — | 80 | — |
| Round of 16 (7 of 8 played) | 7 | — | 35 | — |
| **Total knockout so far** | **23** | **71** | **115** | **62%** |
